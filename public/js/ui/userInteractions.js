...
import { openImageModal, fetchWithCredentials } from './modals.js'; // <-- IMPORTAMOS LA FUNCIÓN
...
async function handleGuestAvatarUpload(event) {
    ...
    try {
        // Usamos la función centralizada que SIEMPRE envía las credenciales
        const result = await fetchWithCredentials('/api/guest/avatar', {
            method: 'POST',
            body: formData,
        });
        // La lógica de éxito/error ya está manejada por la función
    } catch (error) {
