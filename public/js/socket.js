...
export function initializeSocketEvents(socket) {
    ...
    socket.on('reauth_success', () => {
        console.log("Re-autenticación exitosa. Re-uniéndose a la sala...");
        if (state.currentChatContext.type === 'room' && state.currentChatContext.with) {
            socket.emit('join room', { roomName: state.currentChatContext.with });
        }
    });
